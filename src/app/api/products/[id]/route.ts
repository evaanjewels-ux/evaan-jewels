  ...
  try {
    ...
    if (hasCompositionData) {
      ...
      const prices = calculateProductPrice({
        ...
        chargeBasedOnVariant: (validatedData.chargeBasedOnVariant ?? (existing as unknown as Record<string, unknown>).chargeBasedOnVariant) as Parameters<typeof calculateProductPrice>[0]["chargeBasedOnVariant"],
      });
